...
def train_net(data, args, save_dir):
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(solver_opt, T_max=100, eta_min=0.00001)
    ...
    for i in range(nepochs):
        ...
        for Xtrain in train_loader:
            ...
            if not ever_below and errSolver.mean().detach().cpu().numpy() > 1e-2:
                train_loss = torch.cat((train_loss, errSolver * 1e4))
            elif ever_below and errSolver.mean().detach().cpu().numpy() > 1e-2:
                train_loss = torch.cat((train_loss, errSolver * 1e2))
            else:
                ever_below = True
