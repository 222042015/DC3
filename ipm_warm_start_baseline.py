...
class General_QP(object):
    ...
    def warm_start_baseline(self, solver_type='ipopt', tol=1e-4, baseline_method="gauge", problem_type="Simple", num_var=100, num_ineq=50, num_eq=50, sol_path="/home/jxxiong/A-xjx/deeplde/baseline_sols/"):
        ...
        try:
            ...
            print("No such file or directory: ", sol_path)
...
class QCQP(object):
    ...
    def __init__(self, prob_type, learning_type, val_frac=0.0833, test_frac=0.0833, device='cuda:0', seed=17, **kwargs):
        ...
        if prob_type == 'QCQP_H_proj':
            ...
            file_path = kwargs['file_path']
            data = sio.loadmat(file_path)
            self.data_size = data['eq_h'].shape[0]
            ...
            self.num_var = data['Q'].shape[0]
            self.num_ineq = data['ineq_H'].shape[0]
            self.num_eq = data['eq_H'].shape[0]
            self.num_lb = data['Lb'].shape[1]
            self.num_ub = data['Ub'].shape[1]
        elif prob_type == 'Nonconvex_QCQP':
            self.data_size = kwargs['data_size']
            self.train_size = int(self.data_size * self.train_frac)
            self.val_size = int(self.data_size * val_frac)
            self.test_size = self.data_size - self.train_size - self.val_size
            ...
            self.num_lb = 0
            self.num_ub = 0
            self.x_sol = torch.randn(size=(self.data_size, self.num_var, 1), device=self.device)
        ...
        if learning_type == 'train':
            if (prob_type == 'QCQP_H_proj') or (prob_type == 'ACOPF'):
                self.Q = torch.tensor(data['Q'], device=self.device).float().unsqueeze(0).repeat(self.train_size, 1, 1)
                ...
                self.Q = torch.tensor(data['Q'], device=self.device).float()[:self.train_size]
                self.p = torch.tensor(data['p'], device=self.device).float()[:self.train_size].unsqueeze(-1)

            if self.num_eq != 0:
                if prob_type == 'QCQP_H_proj':
                    self.Q_eq = torch.tensor(0.0, device=self.device)
                    self.A = torch.tensor(data['A'], device=self.device).float().unsqueeze(0).repeat(self.train_size, 1, 1)
                    self.b = torch.tensor(data['X'], device=self.device).float()[:self.train_size].unsqueeze(-1)
                elif prob_type == 'ACOPF':
                    self.Q_eq = torch.tensor(data['eq_H'], device=self.device).float()
                    self.A = torch.tensor(data['eq_g'], device=self.device).float().unsqueeze(0).repeat(self.train_size, 1, 1)
                    self.b = torch.tensor(data['eq_h'], device=self.device).float()[:self.train_size].unsqueeze(-1)
                elif prob_type == 'Nonconvex_QCQP':
                    b_list = []
                    self.Q_eq = torch.diag_embed(torch.rand(size=(self.num_eq, self.num_var), device=self.device))
                    #self.A = torch.normal(mean=0, std=1, size=(self.data_size, self.num_eq, self.num_var), device=device)[:self.train_size]
                    self.A = torch.zeros(size=(self.data_size, self.num_eq, self.num_var), device=self.device)[:self.train_size]
                    for i in range(self.num_eq):
                        self.b_i = torch.bmm(self.x_sol[:self.train_size].permute(0,2,1)@self.Q_eq[i,:,:], self.x_sol[:self.train_size])+torch.bmm(self.A[:,i,:].unsqueeze(1), self.x_sol[:self.train_size])
                        b_list.append(self.b_i)
                    self.b = torch.concat(b_list, dim=1)
                    #self.A = torch.zeros(size=(self.data_size, self.num_eq, self.num_var), device=self.device)[:self.train_size]
                    #self.b = torch.rand(size=(self.data_size, self.num_eq), device=device)[:self.train_size].unsqueeze(-1) - 0.5
                    #self.b = torch.zeros(size=(self.data_size, self.num_eq, 1), device=device)[:self.train_size]
                else:
                    try:
                        self.Q_eq = torch.tensor(data['Q_eq'], device=self.device).float()
                    except KeyError:
                        self.Q_eq = torch.tensor(0.0, device=self.device)
                    self.A = torch.tensor(data['A'], device=self.device).float()[:self.train_size]
                    self.b = torch.tensor(data['b'], device=self.device).float()[:self.train_size].unsqueeze(-1)
            ...
            if self.num_ineq != 0:
                if prob_type == 'QCQP_H_proj':
                    ...
                    self.Q_ineq = torch.tensor(data['ineq_H'], device=self.device).float()
                    self.G = torch.tensor(data['ineq_g'], device=self.device).float().unsqueeze(0).repeat(self.train_size, 1,1)
                    self.c = torch.tensor(data['ineq_h'], device=self.device).float().repeat(self.train_size, 1).unsqueeze(-1)
                elif prob_type == 'Nonconvex_QCQP':
                    c_list = []
                    self.Q_ineq = torch.diag_embed(torch.rand(size=(self.num_ineq, self.num_var), device=self.device))
                    self.G = torch.rand(size=(self.data_size, self.num_ineq, self.num_var), device=device)[:self.train_size]
                    for i in range(self.num_ineq):
                        self.c_i = torch.bmm(self.x_sol[:self.train_size].permute(0,2,1)@self.Q_ineq[i,:,:], self.x_sol[:self.train_size])+torch.bmm(self.G[:,i,:].unsqueeze(1), self.x_sol[:self.train_size])
                        c_list.append(self.c_i+1)
                    self.c = torch.concat(c_list, dim=1)
                    #self.G = -torch.rand(size=(self.data_size, self.num_ineq, self.num_var), device=device)[:self.train_size]
                    #self.c = torch.zeros(size=(self.data_size, self.num_ineq, 1), device=self.device)[:self.train_size]
                else:
                    try:
                        self.Q_ineq = torch.tensor(data['Q_ineq'], device=self.device).float()
                    except KeyError:
                        self.Q_ineq = torch.tensor(0.0, device=self.device)
                    self.G = torch.tensor(data['G'], device=self.device).float()[:self.train_size]
                    self.c = torch.tensor(data['c'], device=self.device).float()[:self.train_size].unsqueeze(-1)

            if self.num_lb != 0:
                if prob_type == 'ACOPF':
                    self.lb = torch.tensor(data['Lb'], device=self.device).float().repeat(self.train_size, 1).unsqueeze(-1)
                else:
                    self.lb = torch.tensor(data['lb'], device=self.device).float()[:self.train_size]
            else:
                self.lb = -torch.inf
            ...
            if self.num_ub != 0:
                if prob_type == 'ACOPF':
                    self.ub = torch.tensor(data['Ub'], device=self.device).float().repeat(self.train_size, 1).unsqueeze(-1)
                else:
                    self.ub = torch.tensor(data['ub'], device=self.device).float()[:self.train_size]
            else:
                self.ub = torch.inf
            ...
            if (prob_type == 'QCQP_H_proj') or (prob_type == 'ACOPF'):
                ...
                self.Q = torch.tensor(data['Q'], device=self.device).float()[self.train_size:self.train_size + self.val_size]
                self.p = torch.tensor(data['p'], device=self.device).float()[self.train_size:self.train_size + self.val_size].unsqueeze(-1)

            if self.num_eq != 0:
                if prob_type == 'QCQP_H_proj':
                    self.Q_eq = torch.tensor(0.0, device=self.device).float()
                    self.A = torch.tensor(data['A'], device=self.device).float().unsqueeze(0).repeat(self.val_size, 1, 1)
                    self.b = torch.tensor(data['X'], device=self.device).float()[self.train_size:self.train_size + self.val_size].unsqueeze(-1)
                elif prob_type == 'ACOPF':
                    self.Q_eq = torch.tensor(data['eq_H'], device=self.device).float()
                    self.A = torch.tensor(data['eq_g'], device=self.device).float().unsqueeze(0).repeat(self.val_size, 1, 1)
                    self.b = torch.tensor(data['eq_h'], device=self.device).float()[self.train_size:self.train_size + self.val_size].unsqueeze(-1)
                elif prob_type == 'Nonconvex_QCQP':
                    b_list = []
                    self.Q_eq = torch.diag_embed(torch.rand(size=(self.num_eq, self.num_var), device=self.device))
                    #self.A = torch.normal(mean=0, std=1, size=(self.data_size, self.num_eq, self.num_var), device=device)[self.train_size:self.train_size + self.val_size]
                    self.A = torch.zeros(size=(self.data_size, self.num_eq, self.num_var), device=self.device)[self.train_size:self.train_size + self.val_size]
                    for i in range(self.num_eq):
                        self.b_i = torch.bmm(self.x_sol[self.train_size:self.train_size + self.val_size].permute(0, 2, 1) @ self.Q_eq[i, :, :],
                                             self.x_sol[self.train_size:self.train_size + self.val_size]) + torch.bmm(self.A[:, i, :].unsqueeze(1),
                                                                                      self.x_sol[self.train_size:self.train_size + self.val_size])
                        b_list.append(self.b_i)
                    self.b = torch.concat(b_list, dim=1)
                    #self.A = torch.zeros(size=(self.data_size, self.num_eq, self.num_var), device=self.device)[self.train_size:self.train_size + self.val_size]
                    #self.b = torch.rand(size=(self.data_size, self.num_eq), device=device)[self.train_size:self.train_size + self.val_size].unsqueeze(-1) - 0.5
                    #self.b = torch.zeros(size=(self.data_size, self.num_eq, 1), device=device)[self.train_size:self.train_size + self.val_size]
                else:
                    try:
                        self.Q_eq = torch.tensor(data['Q_eq'], device=self.device).float()
                    except KeyError:
                        self.Q_eq = torch.tensor(0.0, device=self.device).float()
                    self.A = torch.tensor(data['A'], device=self.device).float()[self.train_size:self.train_size + self.val_size]
                    self.b = torch.tensor(data['b'], device=self.device).float()[self.train_size:self.train_size + self.val_size].unsqueeze(-1)
            ...
            if self.num_ineq != 0:
                if prob_type == 'QCQP_H_proj':
                    ...
                    self.Q_ineq = torch.tensor(data['ineq_H'], device=self.device).float()
                    self.G = torch.tensor(data['ineq_g'], device=self.device).float().unsqueeze(0).repeat(self.val_size, 1,1)
                    self.c = torch.tensor(data['ineq_h'], device=self.device).float().repeat(self.val_size, 1).unsqueeze(-1)
                elif prob_type == 'Nonconvex_QCQP':
                    c_list = []
                    self.Q_ineq = torch.diag_embed(torch.rand(size=(self.num_ineq, self.num_var), device=self.device))
                    self.G = torch.rand(size=(self.data_size, self.num_ineq, self.num_var), device=device)[self.train_size:self.train_size + self.val_size]
                    for i in range(self.num_ineq):
                        self.c_i = torch.bmm(self.x_sol[self.train_size:self.train_size + self.val_size].permute(0, 2, 1) @ self.Q_ineq[i, :, :],
                                             self.x_sol[self.train_size:self.train_size + self.val_size]) + torch.bmm(self.G[:, i, :].unsqueeze(1),
                                                                                       self.x_sol[self.train_size:self.train_size + self.val_size])
                        c_list.append(self.c_i + 1)
                    self.c = torch.concat(c_list, dim=1)
                    #self.G = -torch.rand(size=(self.data_size, self.num_ineq, self.num_var), device=device)[self.train_size:self.train_size + self.val_size]
                    #self.c = torch.zeros(size=(self.data_size, self.num_ineq, 1), device=self.device)[self.train_size:self.train_size + self.val_size]
                else:
                    try:
                        self.Q_ineq = torch.tensor(data['Q_ineq'], device=self.device).float()
                    except KeyError:
                        self.Q_ineq = torch.tensor(0.0, device=self.device)
                    self.G = torch.tensor(data['G'], device=self.device).float()[self.train_size:self.train_size + self.val_size]
                    self.c = torch.tensor(data['c'], device=self.device).float()[self.train_size:self.train_size + self.val_size].unsqueeze(-1)

            if self.num_lb != 0:
                if prob_type == 'ACOPF':
                    self.lb = torch.tensor(data['Lb'], device=self.device).float().repeat(self.val_size, 1).unsqueeze(-1)
                else:
                    self.lb = torch.tensor(data['lb'], device=self.device).float()[self.train_size:self.train_size + self.val_size]
            else:
                self.lb = -torch.inf
            if self.num_ub != 0:
                if prob_type == 'ACOPF':
                    self.ub = torch.tensor(data['Ub'], device=self.device).float().repeat(self.val_size, 1).unsqueeze(-1)
                else:
                    self.ub = torch.tensor(data['ub'], device=self.device).float()[self.train_size:self.train_size + self.val_size]
            else:
                self.ub = torch.inf
            ...
            if (prob_type == 'QCQP_H_proj') or (prob_type == 'ACOPF'):
                ...
                self.Q = torch.tensor(data['Q'], device=self.device).float()[self.train_size + self.val_size:]
                self.p = torch.tensor(data['p'], device=self.device).float()[self.train_size + self.val_size:].unsqueeze(-1)
            ...
            if self.num_eq != 0:
                if prob_type == 'QCQP_H_proj':
                    ...
                    self.Q_eq = torch.tensor(data['eq_H'], device=self.device).float()
                    self.A = torch.tensor(data['eq_g'], device=self.device).float().unsqueeze(0).repeat(self.test_size, 1, 1)
                    self.b = torch.tensor(data['eq_h'], device=self.device).float()[self.train_size + self.val_size:].unsqueeze(-1)
                elif prob_type == 'Nonconvex_QCQP':
                    self.Q_eq = torch.diag_embed(torch.randn(size=(self.num_eq, self.num_var), device=self.device)).float()
                    b_list = []
                    #self.A = torch.normal(mean=0, std=1, size=(self.data_size, self.num_eq, self.num_var),device=device)[self.train_size + self.val_size:]
                    self.A = torch.zeros(size=(self.data_size, self.num_eq, self.num_var), device=self.device)[self.train_size + self.val_size:]
                    for i in range(self.num_eq):
                        self.b_i = torch.bmm(
                            self.x_sol[self.train_size + self.val_size:].permute(0, 2, 1) @ self.Q_eq[i, :, :],
                            self.x_sol[self.train_size + self.val_size:]) + torch.bmm(self.A[:, i, :].unsqueeze(1),
                            self.x_sol[self.train_size + self.val_size:])
                        b_list.append(self.b_i)
                    self.b = torch.concat(b_list, dim=1)
                    #self.A = torch.zeros(size=(self.data_size, self.num_eq, self.num_var), device=self.device)[self.train_size + self.val_size:]
                    #self.b = torch.rand(size=(self.data_size, self.num_eq), device=device)[self.train_size + self.val_size:].unsqueeze(-1) - 0.5
                    #self.b = torch.zeros(size=(self.data_size, self.num_eq, 1), device=device)[self.train_size + self.val_size:]
                else:
                    try:
                        self.Q_eq = torch.tensor(data['Q_eq'], device=self.device).float()
                    except KeyError:
                        self.Q_eq = torch.tensor(0.0, device=self.device)
                    self.A = torch.tensor(data['A'], device=self.device).float()[self.train_size + self.val_size:]
                    self.b = torch.tensor(data['b'], device=self.device).float()[self.train_size + self.val_size:].unsqueeze(-1)
            ...
            if self.num_ineq != 0:
                if prob_type == 'QCQP_H_proj':
                    ...
                    self.Q_ineq = torch.tensor(data['ineq_H'], device=self.device).float()
                    self.G = torch.tensor(data['ineq_g'], device=self.device).float().unsqueeze(0).repeat(self.test_size, 1,1)
                    self.c = torch.tensor(data['ineq_h'], device=self.device).float().repeat(self.test_size, 1).unsqueeze(-1)
                elif prob_type == 'Nonconvex_QCQP':
                    c_list = []
                    self.Q_ineq = torch.diag_embed(torch.rand(size=(self.num_ineq, self.num_var), device=self.device))
                    self.G = torch.rand(size=(self.data_size, self.num_ineq, self.num_var), device=device)[self.train_size + self.val_size:]
                    for i in range(self.num_ineq):
                        self.c_i = torch.bmm(
                            self.x_sol[self.train_size + self.val_size:].permute(0, 2, 1) @ self.Q_ineq[i, :, :],
                            self.x_sol[self.train_size + self.val_size:]) + torch.bmm(self.G[:, i, :].unsqueeze(1),
                            self.x_sol[self.train_size + self.val_size:])
                        c_list.append(self.c_i + 1)
                    self.c = torch.concat(c_list, dim=1)
                    #self.G = -torch.rand(size=(self.data_size, self.num_ineq, self.num_var), device=device)[self.train_size + self.val_size:]
                    #self.c = torch.zeros(size=(self.data_size, self.num_ineq, 1), device=self.device)[self.train_size + self.val_size:]
                else:
                    try:
                        self.Q_ineq = torch.tensor(data['Q_ineq'], device=self.device).float()
                    except KeyError:
                        self.Q_ineq = torch.tensor(0.0, device=self.device)
                    self.G = torch.tensor(data['G'], device=self.device).float()[self.train_size + self.val_size:]
                    self.c = torch.tensor(data['c'], device=self.device).float()[self.train_size + self.val_size:].unsqueeze(-1)

            if self.num_lb != 0:
                if prob_type == 'ACOPF':
                    self.lb = torch.tensor(data['Lb'], device=self.device).float().repeat(self.test_size, 1).unsqueeze(-1)
                else:
                    self.lb = torch.tensor(data['lb'], device=self.device).float()[self.train_size + self.val_size:]
            else:
                self.lb = -torch.inf
            ...
            if self.num_ub != 0:
                if prob_type == 'ACOPF':
                    self.ub = torch.tensor(data['Ub'], device=self.device).float().repeat(self.test_size, 1).unsqueeze(-1)
                else:
                    self.ub = torch.tensor(data['ub'], device=self.device).float()[self.train_size + self.val_size:]
            else:
                self.ub = torch.inf
    ...
    def ineq_resid(self, x, **kwargs):
        ...
        if (Q_ineq == 0).all():
            ...
            res = []
            for i in range(self.num_ineq):
                resi = torch.bmm(x.permute(0, 2, 1)@Q_ineq[i, :, :], x) + \
    ...
    def ineq_grad(self, x, **kwargs):
        """
        return: [batch_size, num_ineq, num_var]
        """
        Q_ineq = kwargs.get('Q_ineq', self.Q_ineq)
        G = kwargs.get('G', self.G)
        if (Q_ineq == 0).all():
    ...
    def eq_resid(self, x, **kwargs):
        Q_eq = kwargs.get('Q_eq', self.Q_eq)
        A = kwargs.get('A', self.A)
        b = kwargs.get('b', self.b)
        if (Q_eq == 0).all():
            return torch.bmm(A, x) - b
            ...
            res = []
            for i in range(self.num_eq):
                resi = torch.bmm(x.permute(0, 2, 1) @ Q_eq[i, :, :], x) + \
                    torch.bmm(A[:, i, :].unsqueeze(-1).permute(0, 2, 1), x) - b[:, i, :].unsqueeze(-1)
                res.append(resi)
            return torch.concat(res, dim=1)
    ...
    def eq_dist(self, x, **kwargs):
        ...
        A = kwargs.get('A', self.A)
        b = kwargs.get('b', self.b)
        return torch.abs(self.eq_resid(x, Q_eq=Q_eq, A=A, b=b))
    ...
    def eq_grad(self, x, **kwargs):
        ...
        if (Q_eq == 0).all():
            ...
            grad_list = []
            for i in range(self.num_eq):
                grad_list.append(x.permute(0, 2, 1) @ (Q_eq[i, :, :].T+Q_eq[i,:,:]) + A[:, i, :].unsqueeze(1))
            return torch.concat(grad_list, dim=1)
    ...
    def cal_kkt_info(self, x, eta, s, lamb, zl, zu, sigma, **kwargs):
        ...
        # residual
        r_list = []
        r1 = torch.bmm(0.5*(Q+Q.permute(0,2,1)), x) + p
        if self.num_ineq != 0:
            r1 += torch.bmm(self.ineq_grad(x, Q_ineq=Q_ineq, G=G).permute(0,2,1), eta)
        if self.num_eq != 0:
            r1 += torch.bmm(self.eq_grad(x, Q_eq=Q_eq, A=A, b=b).permute(0,2,1), lamb)
        if self.num_lb != 0:
            r1 += -zl
        if self.num_ub != 0:
            r1 += zu
        ...
        if self.num_ineq != 0:
            r2 = self.ineq_resid(x, Q_ineq=Q_ineq, G=G, c=c) + s
            r3 = eta * s - mu
            r_list.append(r2)
            r_list.append(r3)

        if self.num_eq != 0:
            r4 = self.eq_resid(x, A=A, b=b)
            r_list.append(r4)
        ...
        # jacobian of residual
        H_list = []
        H1 = 0.5*(Q+Q.permute(0,2,1))
        if self.num_ineq != 0:
            H1 = torch.concat((H1, self.ineq_grad(x, Q_ineq=Q_ineq, G=G).permute(0,2,1)), dim=2)
        if self.num_eq != 0:
            H1 = torch.concat((H1, self.eq_grad(x, Q_eq=Q_eq, A=A, b=b).permute(0,2,1)), dim=2)
        if self.num_ineq != 0:
            H1 = torch.concat((H1, torch.zeros(size=(batch_size, self.num_var, self.num_ineq), device=self.device)),
                              dim=2)
        if self.num_lb != 0:
            H1 = torch.concat((H1, -torch.diag_embed(torch.ones(size=(batch_size, self.num_lb), device=self.device))),
                              dim=2)
        if self.num_ub != 0:
            H1 = torch.concat((H1, torch.diag_embed(torch.ones(size=(batch_size, self.num_ub), device=self.device))),
                              dim=2)
        H_list.append(H1)

        if self.num_ineq != 0:
            H2 = torch.concat((self.ineq_grad(x, Q_ineq=Q_ineq, G=G),
                               torch.zeros(size=(batch_size, self.num_ineq, self.num_ineq), device=self.device)), dim=2)
            if self.num_eq != 0:
                H2 = torch.concat((H2, torch.zeros(size=(batch_size, self.num_ineq, self.num_eq), device=self.device)),
                                  dim=2)
            H2 = torch.concat((H2, torch.diag_embed(torch.ones(size=(batch_size, self.num_ineq), device=self.device))),
                              dim=2)
            if self.num_lb != 0:
                H2 = torch.concat((H2, torch.zeros(size=(batch_size, self.num_ineq, self.num_lb), device=self.device)),
                                  dim=2)
            if self.num_ub != 0:
                H2 = torch.concat((H2, torch.zeros(size=(batch_size, self.num_ineq, self.num_ub), device=self.device)),
                                  dim=2)
            H_list.append(H2)
            ...
            H3 = torch.concat((H3, torch.diag_embed(s.squeeze(-1))), dim=2)
            if self.num_eq != 0:
                H3 = torch.concat((H3, torch.zeros(size=(batch_size, self.num_ineq, self.num_eq), device=self.device)),
                                  dim=2)
            H3 = torch.concat((H3, torch.diag_embed(eta.squeeze(-1))), dim=2)
            if self.num_lb != 0:
                H3 = torch.concat((H3, torch.zeros(size=(batch_size, self.num_ineq, self.num_lb), device=self.device)),
                                  dim=2)
            if self.num_ub != 0:
                H3 = torch.concat((H3, torch.zeros(size=(batch_size, self.num_ineq, self.num_ub), device=self.device)),
                                  dim=2)
            H_list.append(H3)

        if self.num_eq != 0:
            H4 = self.eq_grad(x, Q_eq=Q_eq, A=A, b=b)
            if self.num_ineq != 0:
                H4 = torch.concat((H4, torch.zeros(size=(batch_size, self.num_eq, self.num_ineq), device=self.device)),
                                  dim=2)
            H4 = torch.concat((H4, torch.zeros(size=(batch_size, self.num_eq, self.num_eq), device=self.device)), dim=2)
            if self.num_ineq != 0:
                H4 = torch.concat((H4, torch.zeros(size=(batch_size, self.num_eq, self.num_ineq), device=self.device)),
                                  dim=2)
            if self.num_lb != 0:
                H4 = torch.concat((H4, torch.zeros(size=(batch_size, self.num_eq, self.num_lb), device=self.device)),
                                  dim=2)
            if self.num_ub != 0:
                H4 = torch.concat((H4, torch.zeros(size=(batch_size, self.num_eq, self.num_ub), device=self.device)),
                                  dim=2)
            H_list.append(H4)
        ...
        if self.num_lb != 0:
            H5 = torch.diag_embed(zl.squeeze(-1))
            if self.num_ineq != 0:
                H5 = torch.concat((H5, torch.zeros(size=(batch_size, self.num_lb, self.num_ineq), device=self.device)),
                                  dim=2)
            if self.num_eq != 0:
                H5 = torch.concat((H5, torch.zeros(size=(batch_size, self.num_lb, self.num_eq), device=self.device)),
                                  dim=2)
            if self.num_ineq != 0:
                H5 = torch.concat((H5, torch.zeros(size=(batch_size, self.num_lb, self.num_ineq), device=self.device)),
                                  dim=2)
            H5 = torch.concat((H5, torch.diag_embed((x - lb).squeeze(-1))), dim=2)
            if self.num_ub != 0:
                H5 = torch.concat((H5, torch.zeros(size=(batch_size, self.num_lb, self.num_ub), device=self.device)),
                                  dim=2)
            H_list.append(H5)
        ...
        if self.num_ub != 0:
            H6 = -torch.diag_embed(zu.squeeze(-1))
            if self.num_ineq != 0:
                H6 = torch.concat((H6, torch.zeros(size=(batch_size, self.num_ub, self.num_ineq), device=self.device)),
                                  dim=2)
            if self.num_eq != 0:
                H6 = torch.concat((H6, torch.zeros(size=(batch_size, self.num_ub, self.num_eq), device=self.device)),
                                  dim=2)
            if self.num_ineq != 0:
                H6 = torch.concat((H6, torch.zeros(size=(batch_size, self.num_ub, self.num_ineq), device=self.device)),
                                  dim=2)
            if self.num_lb != 0:
                H6 = torch.concat((H6, torch.zeros(size=(batch_size, self.num_ub, self.num_lb), device=self.device)),
                                  dim=2)
            H6 = torch.concat((H6, torch.diag_embed((ub - x).squeeze(-1))), dim=2)
            H_list.append(H6)
    ...
    def opt_solve(self, solver_type='ipopt', tol=1e-4, initial_y = None, init_mu=None, init_g=None, init_zl=None, init_zu=None):
        if solver_type == 'ipopt':
            Q, p = self.Q.detach().cpu().numpy(), self.p.detach().cpu().numpy()
            if self.num_ineq != 0:
                Q_ineq, G, c = self.Q_ineq.detach().cpu().numpy(), self.G.detach().cpu().numpy(), self.c.detach().cpu().numpy()
            if self.num_eq != 0:
                Q_eq, A, b = self.Q_eq.detach().cpu().numpy(), self.A.detach().cpu().numpy(), self.b.detach().cpu().numpy()
            if self.num_lb != 0:
                lb = self.lb.detach().cpu().numpy()
            else:
                lb = -np.infty * np.ones(shape=(Q.shape[0], Q.shape[1], 1))
            ...
            for i in range(Q.shape[0]):
                ...
                if (self.num_ineq != 0) and (self.num_eq != 0):
                    Q_ineq, Q_eq, G0, A0 = Q_ineq, Q_eq, G[i], A[i]
                elif (self.num_ineq != 0) and (self.num_eq == 0):
                    Q_ineq, Q_eq, G0, A0 = Q_ineq, np.array([0.0]), G[i], np.array([0.0])
                elif (self.num_ineq == 0) and (self.num_eq != 0):
                    Q_ineq, Q_eq, G0, A0 = np.array([0.0]), Q_eq, np.array([0.0]), A[i]

                nlp = convex_qcqp_ipopt(
                    Q[i],
                    p[i].squeeze(-1),
                    Q_ineq,
                    Q_eq,
                ...
                y, info = nlp.solve(y0, lagrange=g, zl=zl, zu=zu)
                # y, info = nlp.solve(np.ones(self.num_var)*0.5)
    ...
    def warm_start_baseline(self, solver_type='ipopt', tol=1e-4, baseline_method="gauge", problem_type="Simple", num_var=100, num_ineq=50, num_eq=50, sol_path="/home/jxxiong/A-xjx/deeplde/baseline_sols/"):
        ...
        ineq_dist = self.ineq_dist(Y0.unsqueeze(-1), Q_ineq=self.Q_ineq, G=self.G, c=self.c)
...
class convex_qcqp_ipopt(ipopt.Problem):
    def __init__(self, Q, p, Q_ineq, Q_eq, G, A, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.Q = Q
        self.p = p
        self.Q_ineq = Q_ineq
    ...
    def constraints(self, y):
        ...
        if self.num_eq != 0:
            if (self.Q_eq == 0).all():
                ...
                eq_const = []
                for i in range(self.num_eq):
                    eq_const.append(y.T @ self.Q_eq[i] @ y + self.A[i, :].T @ y)
                const_values.append(np.array(eq_const))
        return np.hstack(const_values)
    ...
    def jacobian(self, y):
        ...
        if self.num_eq != 0:
            if (self.Q_eq == 0).all():
                ...
                eq_grad = []
                for i in range(self.num_eq):
                    eq_grad.append((self.Q_eq[i,:,:].T+self.Q_eq[i,:,:]) @ y + self.A[i, :])
                const_jacob.append(np.concatenate(eq_grad, axis=-1).T.flatten())
        return np.concatenate(const_jacob)
