...
class QPProblem(BaseProblem):
    ...
    def __init__(self, dataset, test_size):
        ...
        self.trainX = self.X[:-2*test_size]
        self.validX = self.X[-2*test_size:-test_size]
        self.testX = self.X[-test_size:]
        self.trainY = self.Y[:-test_size]
        self.validY = self.Y[-2*test_size:-test_size]
...
class QCQPProbem(QPProblem):
    ...
    def ineq_resid(self, X, Y):
        ...
        res = q + torch.matmul(Y, self.G.T) - self.h
        return res
    
    def ineq_dist(self, X, Y):
        res = self.ineq_resid(X, Y)
