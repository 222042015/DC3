...
from pypower.api import case118, case57, case300, case39
...
# ACOPF
###################################################################
CASE_FNS = dict([(300, case300), (57, case57), (118, case118), (39, case39)])
...
class ACOPFProblem:
    """
        ...
        formulation type:
            - 0: the basic model, only consider the upper and lower bound for active and reactive generation
            - 1: also consider the angle difference constraints on the branches
            - 2: also consider the branch flow limits
    """

    def __init__(self, filename, valid_frac=0.0833, test_frac=0.0833, formulation=0):
        ...
        # only consider the branches with active branch limits
        self.bfmax = self.branch[:, RATE_A] / self.baseMVA
        ...
        if formulation == 0:
            self._nineq = 4*self.ng
        elif formulation == 1:
            self._nineq = 4*self.ng + 2*len(self.branch)
        elif formulation == 2:
            self._nineq = 4*self.ng + 2*len(self.branch) + 2*len(self.bfmax)
        else:
        ...
        print("the range of the voltage angle: {}, {}".format(self.va_min, self.va_max))
    ...
    def ineq_resid(self, X, Y):
        ...
        if self.formulation == 2:
            Sf, St = self.get_branch_flow(X, Y)
            resids = torch.cat([
                resids,
                torch.abs(Sf[:, self.limited_branch]) - self.bfmax,
                torch.abs(St[:, self.limited_branch]) - self.bfmax
            ], dim=1)
